...
async function main() {
  ...
  const photoRef = process.argv[3];
  if (!venueId || !photoRef) throw new Error('Usage: node scripts/set-preferred-photo.mjs <venue_id> <photo_reference>');
  ...
  const { error } = await supabase
    .from('venues')
    .update({ preferred_photo_ref: photoRef })
    .eq('id', venueId);
  if (error) throw new Error(error.message);
  console.log('Updated preferred_photo_ref');
}
