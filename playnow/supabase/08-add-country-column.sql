-- Add country column to venues and backfill existing rows

alter table if exists venues
  add column if not exists country text;

-- Backfill existing rows only (no default)
update venues set country = 'Australia' where country is null;

-- Index for country filtering
create index if not exists idx_venues_country on venues(country);

-- Update search_venues RPC to include country in results
